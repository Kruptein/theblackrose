...
use crate::{models::{
    matches::{
        Match, MatchFeedElement, MatchFeedParticipant, ParticipantGeneral, ParticipantItems,
...
pub async fn get_match(conn: &PgPool, game_id: i64) -> Result<Match, Error> {
...
pub async fn get_match_ids(
    conn: &PgPool,
    summoners: Vec<String>,
    queues: Vec<i16>,
    before_time: i64,
    after_time: i64,
    length: i64,
) -> Result<Vec<ParticipantGeneral>, Error> {
...
pub async fn get_game_details(game_id: i64, conn: &PgPool) -> Result<MatchFeedElement, Error> {
    let match_info = get_match(conn, game_id).await?;

    let participants = query_as!(
        MatchFeedParticipant,
        r#"
        SELECT
...
pub async fn add_match_team_stats(
    tx: &mut Transaction<'_, Postgres>,
    game_id: i64,
    stats: &R::Team,
) {
...
pub async fn add_participant_stats(
    tx: &mut Transaction<'_, Postgres>,
    stats: &R::Participant,
