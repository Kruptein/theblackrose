...
pub async fn get_summoner_by_name(conn: &PgPool, name: &str) -> Result<Summoner, Error> {
    query_as!(Summoner, "SELECT * FROM summoners WHERE name = $1", name)
        .fetch_one(conn)
        .await
}
...
pub async fn get_summoner_by_puuid(
    ...
    query_as!(Summoner, "SELECT * FROM summoners WHERE puuid = $1", puuid)
        .fetch_one(executor)
        .await
}
...
pub async fn add_partial_summoner(
    executor: &mut PgConnection,
